...
def main(seq):
    ...
    boundary_snapping_result_path = os.path.join('DAVIS', 'boundary_snappingResults', 'Segmentations', '480p', 'OSVOS', seq_name)
    ...
    if len(result_filenames)==0:
        ...
        print("mean IOU without BS: ", mean_iou_score(annotation_imgs,result_imgs,show_per_frame_iou))
    ...
    if len(boundary_snapping_result_path)==0:
        ...
        print("mean IOU with BS: ", mean_iou_score(annotation_imgs,boundary_snapping_result_path,show_per_frame_iou))
    return scores, count
